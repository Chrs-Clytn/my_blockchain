...
command_t *parse_input(char *input)  // all the functionality (append, remove, list, sync) comes in here!!
{
    ...
    command->node = false;
    command->block = false;
    command->node_id = 0;  // id is third arg and depends on "node" or "block" named before – so just one id needed here
    command->block_id = NULL;
    command->all = false;
    ...
    for (int i = 0; i < input_arr->size - 1; i++)
    {
        ...
        if (input_arr->size > 3)                           
            command->node_id = my_atoi(input_arr->array[2]);

        if (input_arr->size > 4 && my_strcmp("*", input_arr->array[3]) == 0)
            command->all = true;
         // add one for block id, very basic atm just confirms "add" "block" and then takes the next string in the array as block id
        if((my_strcmp("add", input_arr->array[i]) == 0) && (my_strcmp("block", input_arr->array[i]) == 0))
            command->block_id = input_arr->array[2];
    }
